import type { ExtractedPattern, PatternVectorStoreResult, RetrievedPattern, RetrievalFilters } from './types.js';
...
export class CloudflareVectorizePatternStore {
...
export class CloudflareVectorizePatternRetriever {
  constructor(private env: {
    REPORT_PATTERNS: VectorizeIndex;
    AI: Ai;
    PATTERNS_BUCKET?: R2Bucket;
  ...
  async retrieveSimilar(query: string, filters: RetrievalFilters = {}, limit = 10): Promise<RetrievedPattern[]> {
    let vec: number[];
    try {
...
function buildFilter(f: RetrievalFilters): Record<string, any> | undefined {
