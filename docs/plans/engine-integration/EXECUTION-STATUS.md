...
## raaga-clip-generation (p5-p4-next-batch) — executed 2026-07-17 (Codex; worktree only, no push/merge)
**Task:** Implement raaga clip generation (P3 suno-bridge or server-side): fill generated_audio.clip_url (null per T-031) — render strudel_ratios to audio (offline render or external service per goal-understanding), return clip_url (local file/serve or service URL), consent-gated, config-driven. Source: docs/plans/engine-integration/p5-p4-next-batch.json (raaga-clip-generation).
**Worktree:** `.worktrees/raaga-clip-codex` on branch `swarm/engines/p3-w3/raaga/raaga-clip-codex` (from main; edits only here).

**Changes (in worktree only):**
- NEW `ts-engines/src/engines/raaga/clip.ts` — config-driven clip path: `RAAGA_CLIP_MODE=off|local|service` (default `off`), `RAAGA_CLIP_DIR` (default `<os-tmp>/raaga-clips`), `RAAGA_CLIP_BASE_URL` (default `/clips/raaga`), `RAAGA_CLIP_SERVICE_URL` (service mode), `RAAGA_CLIP_SECONDS_PER_NOTE` (default 0.6). `renderRaagaClipWav()` = offline WebAudio-compatible PCM16 mono WAV render of the just-intonation strudel_ratios (ascending arohana, sine voices, raised-cosine fades, 22050 Hz) — no network/secrets (local-first per goal-understanding.md). `storeClip()` content-addressed (`raaga-m<num>-r<rootHz>-<sha1-10>.wav`, idempotent), `resolveStoredClip()` path-traversal-safe. Service mode POSTs {melakarta, ratios, root_hz} to the configured endpoint with 10 s timeout and degrades gracefully (`service_unavailable` + null clip_url) — suno-bridge compatible (existing /suno proxy in app.ts) but not hardcoded (kimi/prod URLs unresolved per goal-understanding §7).
- `ts-engines/src/engines/raaga/engine.ts` — new `request_clip` input param (boolean, default false); consent-gated clip block: request_clip + granted consent (top-level `consent` or `parameters.consent`, FROZEN Consent shape) → `generateRaagaClip()`; populates `generated_audio.clip_url` + `generated_audio.metadata.clip` note {requested, status: not_requested|consent_missing|skipped_off|generated|service_unavailable, mode, detail}. clip_url stays null when not requested / consent missing / mode off (backward compat per FROZEN). `strudel_ratios: [...m.ratios]` spread fixes pre-existing readonly TS2322.
- `ts-engines/src/server/app.ts` — NEW static route `GET /clips/raaga/:file` serving stored WAVs from the clip store (audio/wav, immutable cache, 404 for absent/invalid, traversal-guarded).
- NEW `ts-engines/src/engines/raaga/engine.test.ts` — 12 bun tests: mode resolution; WAV header/size; store + traversal safety; not-requested → null; consent absent → null + consent_missing; consent not granted → null + consent_missing; mode off + request + consent → null + skipped_off; local mode + request + granted consent → clip_url populated + WAV on disk; parameters-nested consent compat; service unreachable → graceful null; HTTP surface serves clip + 404s + rejects traversal.

**Tests (all green):** `cd .worktrees/raaga-clip-codex/ts-engines && bun test` → 87 pass / 1 fail where the 1 fail is the documented pre-existing 30 s real-NVIDIA sigil timeout (identical on main baseline, verified in temp baseline worktree: 20 pass / same 1 fail). New raaga file: 12/12 pass. Typecheck: `bunx typescript@5.6.3 --noEmit` at exact parity with main baseline (pre-existing errors only in app.ts info route + baseline_registry.test.ts; raaga engine.ts pre-existing TS2322 now fixed). (bunx tsc version drift 5.6.3 vs 7.0.2 noted; pinned 5.6.3 for parity.)

**Cites (enforced in code headers/tests):** p1-w1-worker-bootstrap-packet.md + resources-and-assets.md (raaga production-ready; Strudel client-side) + gaps-and-improvements.md (§4: server-side clip generation/storage unclear — now closed locally) + goal-understanding.md (local-first + explicit consent; prod URLs unresolved → config-driven) + EXECUTION-STATUS.md + P1W1-W2-HANDOFF.md + .worktrees/T-002-copilot/.../P1W1-CONTRACTS-FROZEN.md (generated_audio: strudel_ratios + clip_url) + detailed-task-list.md + p5-p4-next-batch.json + ts-engines/src/engines/raaga/* (T-031 merged).
**Tags:** phase:integration-p1 wave:integration-w2 area:engine-integration engine-raaga.
**Status:** Complete in isolated worktree. clip_url populated via consent-gated, config-driven generation path; null default preserved (backward compat per FROZEN). No push/merge. Ready for wave-boundary review/merge.
