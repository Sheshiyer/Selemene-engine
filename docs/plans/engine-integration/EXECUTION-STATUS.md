...
| T-094 | Separate privacy-safe P4 dependency health for 4 focus engines, 2 Python sidecars, and configured providers | backend / codex | main @ `a45952482` (source `0430cff9`) | ✅ COMPLETE + MERGED | #898 subset | Added `GET /health/dependencies`; legacy `/health`, `/health/live`, and `/ready` remain unchanged. Response is an exact allowlist (`status`, `engines`, `sidecars`, `providers`; entries `id/status/reason`), omits URLs/tokens/raw errors, probes sidecars concurrently with ≤500 ms bounds, and lists only providers with non-empty configuration. Evidence on merged main: P4 health 2/2, routing 20/20, API lib 83/83, Clippy `-D warnings` clean. The earlier `p4-health-codex` worktree was preserved but rejected because it changed liveness semantics and leaked internals. |
| P4 API + bridge recovery | Carry merged SDK top-level media through Rust API/auth/orchestrator and the real native/TS/Python boundaries | backend / codex | main @ `a45952482` (source `0430cff9`) | ✅ COMPLETE + MERGED | #898 subset | Four frozen SDK fixtures cross HTTP normalization and exact consent scopes; legacy `options` still work; authentication overrides caller consciousness hints; multipart media is lossless base64; generated media is available at both SDK top-level and legacy `result.generated_*` paths; TS focus is exactly raaga + sigil-forge while biofield + face-reading remain native and live biofield analysis remains direct Python `/analyze`. Evidence: P4 media routes 5/5, bridge 40/40, SDK 27/27, TS focus 33/33, JSON schemas 15/15, all-target check clean. Billing E2E remains unvalidated because absent local Postgres reproduces the identical four pre/post `PoolTimedOut` failures; no billing paths changed. |
| P5 trusted desktop gateway + engine-to-Noesis continuity | Package the SDK, move protected transport into Electron main/preload, implement real consented Biofield capture, and continue safe focus-engine results into Noesis | backend + desktop / codex | Selemene integration @ `d4f071434`; Sankalpa main @ `8692370` (integration `7e08431`, source `7002d5f`) | ✅ COMPLETE + MERGED | T-130 / T-135 wave | `@selemene/engine-sdk` now emits packable ESM/declarations and supports authenticated Biofield session/capture; Raaga clip requests require exact `raaga-audio` consent. Sankalpa consumes the 20.4 kB packed artifact, keeps URL/auth/timeout and allowlisted IPC in Electron main, rejects malformed/unauthenticated/wrong-scope requests before network, removes backend origins from renderer CSP, calculates camera metrics from actual pixels, and normalizes four bounded/redacted result artifacts into provenance-labelled Noesis witness routes. Evidence: SDK 33/33 plus typecheck/build/pack; clean Sankalpa install, 66/66 tests, renderer+Electron typecheck, production build, unsigned arm64 package, production audit 0; browser Gate 4 PASS at 1440x1000; final read-only boundary review GO. Merged-main Vitest discovery excludes nested `.worktrees`, so the canonical suite remains 66 tests. External Temperance rail produced no artifacts and the same wave was completed through its documented fail-open dispatch. |
