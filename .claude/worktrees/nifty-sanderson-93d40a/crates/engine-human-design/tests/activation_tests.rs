...
use engine_human_design::{
    calculate_design_sun_earth, calculate_personality_sun_earth, calculate_sun_earth_activations,
    EphemerisCalculator,
};
...
fn test_personality_sun_earth_basic() {
    ...
    if let Ok((sun, earth)) = result {
        ...
        assert!(
            (diff - 180.0).abs() < 0.01,
            "Earth should be 180° from Sun, got {:.4}° difference",
            diff
        );
...
fn test_complete_sun_earth_activations() {
    ...
    if let Ok(((pers_sun, pers_earth), (des_sun, des_earth))) = result {
        ...
        assert!(
            (pers_diff - 180.0).abs() < 0.01,
            "Personality Earth not 180° from Sun: {:.4}°",
            pers_diff
        );
        assert!(
            (des_diff - 180.0).abs() < 0.01,
            "Design Earth not 180° from Sun: {:.4}°",
            des_diff
        );
...
fn test_design_vs_personality_difference() {
    ...
    if let Ok(((pers_sun, _pers_earth), (des_sun, _des_earth))) = result {
        ...
        assert!(
            sun_diff > 10.0,
            "Sun should have moved significantly between Design and Personality, got {:.2}°",
            sun_diff
        );
