...
use engine_human_design::{generate_hd_chart, Authority, Center, Definition, HDType};
            ...
            match chart.hd_type {
                HDType::Reflector => {
                    assert_eq!(
                        chart.authority,
                        Authority::Lunar,
                        "Reflector should have Lunar authority"
                    );
                    assert_eq!(
                        chart.definition,
                        Definition::NoDefinition,
                        "Reflector should have no definition"
                    );
                }
                ...
                HDType::Manifestor | HDType::Projector => {
                    ...
                    assert!(
                        !sacral.defined,
                        "Manifestor/Projector should NOT have Sacral defined"
                    );
                }
            }
        }
...
fn test_generator_chart() {
    ...
    if let Ok(chart) = generate_hd_chart(birth_time, "") {
        ...
        println!(
            "Sacral defined: {:?}",
            chart.centers.get(&Center::Sacral).map(|c| c.defined)
        );
...
fn test_center_definition_logic() {
    ...
    if let Ok(chart) = generate_hd_chart(birth_time, "") {
        let defined_centers: Vec<_> = chart
            .centers
            .iter()
...
fn test_channel_activation() {
    ...
    if let Ok(chart) = generate_hd_chart(birth_time, "") {
        ...
        for channel in &chart.channels {
            ...
            println!(
                "Channel: {}-{} ({})",
                channel.gate1, channel.gate2, channel.name
            );
        }
    }
}
...
fn test_authority_hierarchy() {
    ...
    if let Ok(chart) = generate_hd_chart(birth_time, "") {
        ...
        let solar_plexus_defined = chart
            .centers
            .get(&Center::SolarPlexus)
            .map(|c| c.defined)
            .unwrap_or(false);
        let _sacral_defined = chart
            .centers
            .get(&Center::Sacral)
            .map(|c| c.defined)
            .unwrap_or(false);
