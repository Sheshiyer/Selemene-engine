...
fn main() {
    ...
    println!(
        "  Data path: '{}' (using built-in ephemeris)\n",
        calc.data_path()
    );
    ...
    match calc.get_all_planets(&test_date) {
        Ok(positions) => {
            ...
            for (planet, pos) in &positions {
                println!(
                    "  {:12?}: {:8.3}° (lat: {:6.3}°, dist: {:.6} AU)",
                    planet, pos.longitude, pos.latitude, pos.distance
                );
            }

            let all_valid = positions
                .iter()
                .all(|(_, pos)| pos.longitude >= 0.0 && pos.longitude < 360.0);
            ...
            let north = positions
                .iter()
                .find(|(p, _)| matches!(p, HDPlanet::NorthNode));
            let south = positions
                .iter()
                .find(|(p, _)| matches!(p, HDPlanet::SouthNode));

            if let (Some((_, n)), Some((_, s))) = (north, south) {
                let diff = (n.longitude - s.longitude + 360.0) % 360.0;
                if (diff - 180.0).abs() < 0.1 {
                    println!(
                        "  ✓ South Node correctly opposite North Node ({:.3}° difference)",
                        (diff - 180.0).abs()
                    );
                } else {
