...
use engine_human_design::{calculate_sun_earth_activations, EphemerisCalculator};
...
fn main() {
    ...
    match calculate_sun_earth_activations(&birth_time, &calculator) {
        Ok(((pers_sun, pers_earth), (des_sun, des_earth))) => {
            ...
            println!("========================================");
            println!(
                "Sun   → Gate {}.{} at {:.4}° longitude",
                pers_sun.gate, pers_sun.line, pers_sun.longitude
            );
            println!(
                "Earth → Gate {}.{} at {:.4}° longitude",
            ...
            println!(
                "Personality: Earth is {:.4}° from Sun (should be 180°)",
                pers_diff
            );
            println!(
                "Design: Earth is {:.4}° from Sun (should be 180°)",
                des_diff
            );
            ...
            println!(
                "\nSun moved {:.2}° between Design and Personality",
                sun_movement
            );
    ...
    let test_dates = vec![
        (
            "Spring Equinox 2000",
            Utc.with_ymd_and_hms(2000, 3, 20, 7, 35, 0).unwrap(),
        ),
        (
            "Summer Solstice 2000",
            Utc.with_ymd_and_hms(2000, 6, 21, 1, 48, 0).unwrap(),
        ),
        (
            "Random Date 1995",
            Utc.with_ymd_and_hms(1995, 11, 23, 18, 15, 0).unwrap(),
        ),
    ];
