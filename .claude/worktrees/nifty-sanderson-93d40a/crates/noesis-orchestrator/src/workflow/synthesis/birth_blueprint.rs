        ...
        if let Some(ref num) = numerology {
            for (theme_name, description) in num.themes() {
                let theme = theme_map
                    ...
                    .or_insert_with(|| Theme::new(theme_name.clone(), description.clone()));
...
fn correlate_life_path_and_type(life_path: u8, hd_type: &str) -> Option<Alignment> {
    ...
    let correlation = match (life_path, hd_type) {
        (1, "Manifestor") | (8, "Manifestor") => Some((
            "Leadership alignment",
            "Both systems emphasize initiating and authority",
        )),
        (2, "Projector") | (6, "Projector") => Some((
            "Guiding alignment",
            "Both systems emphasize guiding and supporting others",
        )),
        (3, "Manifesting Generator") | (5, "Manifesting Generator") => Some((
            "Dynamic creativity",
            "Both systems emphasize creative adaptability",
        )),
        (4, "Generator") | (7, "Generator") => Some((
            "Deep work alignment",
            "Both systems emphasize sustained focus and mastery",
        )),
        (9, "Projector") => Some((
            "Humanitarian guidance",
            "Both systems point to service through wisdom",
        )),
        _ => None,
    };
...
fn correlate_expression_and_profile(expression: u8, profile: &str) -> Option<Alignment> {
    ...
    let correlation = match (expression, profile_lines[0], profile_lines[1]) {
        (1, "1", _) | (8, "1", _) => Some((
            ...
            "Both emphasize research and authority",
        )),
        (3, _, "3") | (5, _, "3") => Some((
            "Experiential learning",
            "Both emphasize learning through experience",
        )),
        (7, "2", _) => Some(("Natural wisdom", "Both emphasize inner knowing and retreat")),
        (6, "4", _) => Some((
            "Community orientation",
            "Both emphasize networks and relationships",
        )),
        _ => None,
    };
    ...
    let correlation = match vim.current_mahadasha_lord.as_str() {
        "Sun"
            if hd
                .defined_centers
                .iter()
                .any(|c| c.contains("G") || c.contains("Identity")) =>
        {
            Some((
                "Identity activation",
                "Sun period amplifies your defined sense of self",
            ))
        }
        "Moon"
            if hd
                .defined_centers
                .iter()
                .any(|c| c.contains("Solar") || c.contains("Emotional")) =>
        {
            Some((
                "Emotional mastery",
                "Moon period deepens emotional intelligence",
            ))
        }
        "Mercury" if hd.defined_centers.iter().any(|c| c.contains("Throat")) => Some((
            "Communication power",
            "Mercury period enhances your natural voice",
        )),
        "Jupiter"
            if hd
                .defined_centers
                .iter()
                .any(|c| c.contains("Ajna") || c.contains("Mind")) =>
        {
            Some((
                "Wisdom expansion",
                "Jupiter period expands your mental gifts",
            ))
        }
        "Saturn" if hd.defined_centers.iter().any(|c| c.contains("Root")) => Some((
            "Grounded discipline",
            "Saturn period strengthens your foundation",
        )),
        _ => None,
    };
...
fn generate_summary(
    ...
    if !alignments.is_empty() {
        ...
        parts.push(format!("Systems align on: {}", alignment_names.join(", ")));
    }
