...
pub fn assess_frequencies(
    ...
    for activation in &gene_keys_chart.active_keys {
        ...
        if let Some(gene_key) = get_gene_key(key_number) {
            let suggested_frequency = consciousness_level.and_then(suggest_frequency_from_level);
...
fn generate_shadow_prompts(gene_key: &GeneKey) -> Vec<String> {
    ...
    vec![
        format!(
            "Do you notice yourself reacting unconsciously through the pattern of {}?",
            shadow_name
        ),
        format!(
            "When {} arises, can you feel the contraction or resistance in your body?",
            shadow_name
        ),
        format!(
            "Do you find yourself caught in {} without realizing it until later?",
            shadow_name
        ),
...
fn generate_gift_prompts(gene_key: &GeneKey) -> Vec<String> {
    ...
    vec![
        format!(
            "When do you experience {} arising naturally, without effort?",
            gift_name
        ),
        format!(
            "Can you notice the space between the {} pattern and the emergence of {}?",
            shadow_name, gift_name
        ),
        format!(
            "What happens when {} flows through you? How does it feel different from the shadow?",
            gift_name
        ),
...
fn generate_siddhi_prompts(gene_key: &GeneKey) -> Vec<String> {
    ...
    vec![
        format!(
            "Have you had moments where {} dissolved into {}?",
            gift_name, siddhi_name
        ),
        format!(
            "When {} is present, is there still a 'you' experiencing it?",
            siddhi_name
        ),
        format!(
            "What remains when the personal expression of {} falls away?",
            gift_name
        ),
        "Have you touched a state beyond your individual consciousness?".to_string(),
        "Can you rest in the paradox of this frequency without needing to understand it?"
            .to_string(),
    ]
}
