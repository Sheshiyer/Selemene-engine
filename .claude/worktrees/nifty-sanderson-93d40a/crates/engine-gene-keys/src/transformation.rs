...
fn create_shadow_to_gift_pathway(gene_key: &GeneKey) -> TransformationPathway {
    ...
    let contemplations = vec![
        format!(
            "Notice when {} arises. Can you feel it in your body before your mind names it?",
            shadow
        ),
        format!(
            "What is {} protecting? What fear lies beneath the pattern?",
            shadow
        ),
        format!(
            "When the {} pattern softens, what quality naturally emerges?",
            shadow
        ),
        "Can you stay present with discomfort without reacting or fixing?".to_string(),
        format!(
            "What would {} look like if it weren't a problem to solve?",
            shadow
        ),
    ];
...
fn create_gift_to_siddhi_pathway(gene_key: &GeneKey) -> TransformationPathway {
    ...
    let contemplations = vec![
        format!(
            "Notice when {} is active. Is there effort? Is there a 'you' doing it?",
            gift
        ),
        format!(
            "What happens when {} operates without personal identity attached?",
            gift
        ),
        format!(
            "Can you feel the space where {} dissolves into {}?",
            gift, siddhi
        ),
        "What remains when this gift serves without anyone serving?".to_string(),
        format!(
            "Is {} a personal achievement or a universal expression?",
            gift
        ),
    ];
...
fn create_siddhi_integration_pathway(gene_key: &GeneKey) -> TransformationPathway {
    ...
    let contemplations = vec![
        format!(
            "Notice when {} is simply present, without arrival or achievement.",
            siddhi
        ),
        format!(
            "Can {} be ordinary? What happens when the sacred becomes everyday?",
            siddhi
        ),
        "Is there integration still needed, or does the question itself dissolve?".to_string(),
        format!(
            "What does {} serve in this moment, through this form?",
            siddhi
        ),
    ];
    ...
    let witnessing_practices = vec![
        format!(
            "You might rest in {} without needing to understand or explain it.",
            siddhi
        ),
        format!(
            "You might allow {} to move through daily life, moment by moment.",
            siddhi
        ),
...
mod tests {
    ...
    fn create_test_chart() -> GeneKeysChart {
        GeneKeysChart {
            ...
            active_keys: vec![GeneKeyActivation {
                key_number: 17,
                line: 3,
