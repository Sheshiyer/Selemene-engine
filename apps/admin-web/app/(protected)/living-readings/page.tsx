...
const READING_TYPES = ["solo", "synastry"] as const;

type ReadingTypeFilter = (typeof READING_TYPES)[number];

function readingTypeFilter(value: string): ReadingTypeFilter | undefined {
  return READING_TYPES.find((candidate) => candidate === value);
}
...
export default function LivingReadingsPage() {
  ...
  const humanQuery = getStringParam(searchParams, "query");
  const readingType = readingTypeFilter(getStringParam(searchParams, "reading_type")) ?? "";
  ...
  const loadData = useCallback(async () => {
    return getAdminLivingReadings(getAuthToken() ?? undefined, {
      query: getStringParam(searchParams, "query") || undefined,
      reading_type: readingTypeFilter(getStringParam(searchParams, "reading_type")),
    ...
    async function run() {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setCompletedTotal(result.completed_total);
        }
      } catch (err) {
        if (!cancelled) {
          setError(
  ...
  const advancedFiltersActive = Boolean(
    ownerUserId || subjectId || relationshipId || sourceId || importRunId
  );
  ...
  return (
    ...
    >
      ...
      <div className="panel-inline">
        <label>
          ...
          <input
            value={humanQuery}
            onChange={(event) =>
              updateQuery({ query: event.target.value || undefined, offset: undefined })
            }
            placeholder="title, reading ID, owner, subject, or relationship"
          />
        </label>
        <label>
          Reading kind
          <select
            value={readingType}
            onChange={(event) =>
              updateQuery({ reading_type: event.target.value || undefined, offset: undefined })
            }
          >
      ...
      <details className="advanced-filters" open={advancedFiltersActive || undefined}>
      ...
      <div className="grid metrics" aria-label="Living reading totals">
        <article className="metric">
          <div className="label">Matching readings</div>
          <div className="value">{loading ? "--" : total}</div>
        </article>
        <article className="metric">
          <div className="label">Completed</div>
          <div className="value">{loading ? "--" : completedTotal}</div>
        </article>
        <article className="metric">
          <div className="label">Showing</div>
          <div className="value">
            {loading ? "--" : `${pageStart}\u2013${pageEnd}`}
          </div>
          <div className="metric-detail">Page size {limit}</div>
        </article>
      </div>
              ...
              <tbody>
                  ...
                  <tr key={item.id}>
                    <td>
                      <Link
                        ...
                        href={`/living-readings/${item.id}`}
                      >
                        {item.title}
                      </Link>
                      <div className="helper">
                        {item.stable_reading_id} · {item.reading_type}
                      </div>
                      {item.summary ? <div className="helper">{item.summary}</div> : null}
                    </td>
                    <td>
                      {item.owner_email}
                    ...
                    <td>
                      <span className="pill">{item.publication_availability.replaceAll("_", " ")}</span>
                    </td>
                ...
                {items.length === 0 ? (
                  <TableEmptyStateRow
                    ...
                    description="Adjust the human search or advanced archive filters."
                  />
                ) : null}
              </tbody>
            </table>
