...
import { ENGINE_IDS, NoesisClient, SelemeneError, WORKFLOW_IDS, } from "./index.js";
const input = {
    birth_data: {
        date: "1991-08-13",
        time: "13:31",
...
describe("NoesisClient", () => {
    it("supports all 16 engine calculate calls", async () => {
        const fetchMock = vi.fn(async () => new Response(JSON.stringify({ engine_id: "ok", result: {} }), { status: 200 }));
    ...
    it("supports all 6 workflow execute calls", async () => {
        const fetchMock = vi.fn(async () => new Response(JSON.stringify({ workflow_id: "ok", engine_outputs: [] }), { status: 200 }));
        vi.stubGlobal("fetch", fetchMock);
        const client = new NoesisClient("https://example.com", { authToken: "token" });
    ...
    it("retries on 5xx and then succeeds", async () => {
        const fetchMock = vi
            ...
            .mockResolvedValueOnce(new Response(JSON.stringify({ engine_id: "numerology", result: {} }), {
            status: 200,
            headers: {
    ...
    it("supports AbortController cancellation", async () => {
        ...
        controller.abort();
        await expect(client.health({ signal: controller.signal })).rejects.toBeInstanceOf(DOMException);
    });
});
