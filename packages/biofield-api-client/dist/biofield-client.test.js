...
describe("BiofieldClient", () => {
    it("creates sessions against the frozen route namespace", async () => {
        ...
        const client = new BiofieldClient("https://example.com", { fetchImpl: fetchMock });
        const session = await client.createSession({ viewer_version: "0.1.0" });
        expect(session.id).toBe("session-1");
        expect(fetchMock).toHaveBeenCalledWith("https://example.com/api/v1/biofield/sessions", expect.objectContaining({
            method: "POST",
        }));
    });
    it("lists readings with query params", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("https://example.com/api/v1/biofield/readings?limit=10&offset=20", expect.objectContaining({
            method: "GET",
        }));
    });
    it("raises a typed error on failed requests", async () => {
        const fetchMock = vi.fn(async () => new Response(JSON.stringify({ error: "nope" }), { status: 503 }));
        const client = new BiofieldClient("https://example.com", { fetchImpl: fetchMock });
        await expect(client.getReading("reading-1")).rejects.toBeInstanceOf(BiofieldClientError);
    });
});
//# sourceMappingURL=biofield-client.test.js.map