...
fn test_wisdom_data_loads() {
    ...
    assert!(
        !INCARNATION_CROSSES.is_empty(),
        "Incarnation crosses should not be empty"
    );
    ...
    assert!(
        !PLANETARY_ACTIVATIONS.is_empty(),
        "Planetary activations should not be empty"
    );
    println!(
        "✓ Loaded {} planetary activations",
        PLANETARY_ACTIVATIONS.len()
    );
}
...
fn test_authority_wisdom_sample() {
    ...
    let sacral_auth = AUTHORITIES
        .get("Sacral_Authority")
