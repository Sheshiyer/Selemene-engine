...
pub fn calculate_design_sun_earth(
    ...
    let design_time =
        calculate_design_time(*birth_time, Some(calculator.data_path())).map_err(|e| {
            EngineError::CalculationError(format!("Design time calculation failed: {}", e))
        })?;
...
type SunEarthActivations = ((Activation, Activation), (Activation, Activation));

pub fn calculate_sun_earth_activations(
    birth_time: &DateTime<Utc>,
    calculator: &EphemerisCalculator,
) -> Result<SunEarthActivations, EngineError> {
    ...
    fn test_earth_opposite_sun() {
        ...
        match calculate_personality_sun_earth(&birth_time, &calculator) {
            Ok((sun, earth)) => {
                ...
                println!(
                    "Personality Sun: Gate {}.{} at {:.4}°",
                    sun.gate, sun.line, sun.longitude
                );
                println!(
                    "Personality Earth: Gate {}.{} at {:.4}°",
                    earth.gate, earth.line, earth.longitude
                );
            }
            Err(e) => {
        ...
        match calculate_sun_earth_activations(&birth_time, &calculator) {
            Ok(((pers_sun, pers_earth), (des_sun, des_earth))) => {
                ...
                assert!(
                    ...
                    "Personality Earth not opposite Sun"
                );
                assert!(
                    (des_diff - 180.0).abs() < 0.001,
                    "Design Earth not opposite Sun"
                );
    ...
    fn test_gate_line_validity() {
        ...
        match calculate_sun_earth_activations(&birth_time, &calculator) {
            Ok(((pers_sun, pers_earth), (des_sun, des_earth))) => {
                ...
                assert!(
                    pers_sun.gate >= 1 && pers_sun.gate <= 64,
                    "Personality Sun gate out of range"
                );
                assert!(
                    pers_sun.line >= 1 && pers_sun.line <= 6,
                    "Personality Sun line out of range"
                );
                assert!(
                    pers_earth.gate >= 1 && pers_earth.gate <= 64,
                    "Personality Earth gate out of range"
                );
                assert!(
                    pers_earth.line >= 1 && pers_earth.line <= 6,
                    "Personality Earth line out of range"
                );

                // Check Design activations
                assert!(
                    des_sun.gate >= 1 && des_sun.gate <= 64,
                    "Design Sun gate out of range"
                );
                assert!(
                    des_sun.line >= 1 && des_sun.line <= 6,
                    "Design Sun line out of range"
                );
                assert!(
                    des_earth.gate >= 1 && des_earth.gate <= 64,
                    "Design Earth gate out of range"
                );
                assert!(
                    des_earth.line >= 1 && des_earth.line <= 6,
                    "Design Earth line out of range"
                );
            }
            Err(e) => {
                println!("Expected: Swiss Ephemeris data not available: {:?}", e);
        ...
        match calculate_personality_activations(&birth_time, &calculator) {
            Ok(activations) => {
                ...
                for act in &activations {
                    println!(
                        "  {:?}: Gate {}.{} at {:.4}°",
                        act.planet, act.gate, act.line, act.longitude
                    );
                }
            }
            Err(e) => {
    ...
    fn test_all_26_activations() {
        ...
        match calculate_all_activations(birth_time, "") {
            Ok((personality, design)) => {
                ...
                let pers_sun_earth_diff =
                    (personality[1].longitude - personality[0].longitude + 360.0) % 360.0;
                assert!(
                    (pers_sun_earth_diff - 180.0).abs() < 0.001,
                    "Personality Earth not opposite Sun"
                );

                let des_sun_earth_diff =
                    (design[1].longitude - design[0].longitude + 360.0) % 360.0;
                assert!(
                    (des_sun_earth_diff - 180.0).abs() < 0.001,
                    "Design Earth not opposite Sun"
                );
    ...
    fn test_personality_sun_earth_match_agent19() {
        ...
        match (
            ...
            (Ok((sun_old, earth_old)), Ok(all_activations)) => {
                ...
                assert_eq!(
                    sun_old.longitude, sun_new.longitude,
                    "Sun longitude mismatch"
                );
                assert_eq!(sun_old.gate, sun_new.gate, "Sun gate mismatch");
                assert_eq!(sun_old.line, sun_new.line, "Sun line mismatch");

                assert_eq!(
                    earth_old.longitude, earth_new.longitude,
                    "Earth longitude mismatch"
                );
