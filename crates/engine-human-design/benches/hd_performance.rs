...
use engine_human_design::{
    analyze_centers, analyze_channels, calculate_all_activations,
    calculate_personality_activations, calculate_profile, determine_authority, determine_type,
    generate_hd_chart, initialize_ephemeris, longitude_to_gate_and_line,
    models::{Activation, Center, CenterState, Channel, Planet},
    ConsciousnessEngine, EngineInput, EphemerisCalculator, HDPlanet, HumanDesignEngine,
};
...
fn bench_26_activations(c: &mut Criterion) {
    ...
    c.bench_function("hd_26_activations", |b| {
        b.iter(|| black_box(calculate_all_activations(black_box(birth_time), "")))
    });
}
...
fn bench_personality_activations(c: &mut Criterion) {
    ...
    c.bench_function("hd_13_personality_activations", |b| {
        b.iter(|| {
            black_box(calculate_personality_activations(
...
fn bench_type_determination(c: &mut Criterion) {
    ...
    c.bench_function("hd_type_determination", |b| {
        b.iter(|| black_box(determine_type(black_box(&centers), black_box(&channels))))
    });
}
...
fn bench_channel_detection(c: &mut Criterion) {
    ...
    c.bench_function("hd_channel_detection", |b| {
        b.iter(|| black_box(analyze_channels(black_box(&activations))))
    });
}
...
fn bench_center_definition(c: &mut Criterion) {
    ...
    c.bench_function("hd_center_definition", |b| {
        b.iter(|| black_box(analyze_centers(black_box(&activations))))
    });
}
...
fn bench_ephemeris_moon_lookup(c: &mut Criterion) {
    ...
    c.bench_function("hd_ephemeris_moon_lookup", |b| {
        b.iter(|| black_box(calculator.get_planet_position(HDPlanet::Moon, black_box(&birth_time))))
    });
}
...
fn bench_ephemeris_sun_lookup(c: &mut Criterion) {
    ...
    c.bench_function("hd_ephemeris_sun_lookup", |b| {
        b.iter(|| black_box(calculator.get_planet_position(HDPlanet::Sun, black_box(&birth_time))))
    });
}
...
fn bench_chart_generation(c: &mut Criterion) {
    ...
    c.bench_function("hd_chart_generation", |b| {
        b.iter(|| black_box(generate_hd_chart(black_box(birth_utc), "")))
    });
}
