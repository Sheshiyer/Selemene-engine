...
fn main() {
    ...
    let test_cases = vec![
        ...
        (
            "Projector Test 1",
            Utc.with_ymd_and_hms(1990, 3, 21, 8, 15, 0).unwrap(),
        ),
        // Case 3: Manifestor
        (
            "Manifestor Test 1",
            Utc.with_ymd_and_hms(1978, 11, 5, 22, 45, 0).unwrap(),
        ),
        // Case 4: Reflector (rare - try multiple)
        (
            "Reflector Test 1",
            Utc.with_ymd_and_hms(1995, 2, 14, 3, 20, 0).unwrap(),
        ),
        // Case 5: Manifesting Generator
        (
            "MG Test 1",
            Utc.with_ymd_and_hms(1988, 9, 9, 18, 0, 0).unwrap(),
        ),
        // Case 6: Another Generator
        (
            "Generator Test 2",
            Utc.with_ymd_and_hms(1975, 4, 10, 6, 30, 0).unwrap(),
        ),
        // Case 7: Another Projector
        (
            "Projector Test 2",
            Utc.with_ymd_and_hms(1992, 7, 25, 16, 45, 0).unwrap(),
        ),
        // Case 8: Another Manifestor
        (
            "Manifestor Test 2",
            Utc.with_ymd_and_hms(1980, 12, 1, 10, 15, 0).unwrap(),
        ),
        // Case 9: Another MG
        (
            "MG Test 2",
        ...
        (
            "Generator Test 3",
            Utc.with_ymd_and_hms(1970, 8, 8, 20, 20, 0).unwrap(),
        ),
        // Case 11: Another Projector
        (
            "Projector Test 3",
            Utc.with_ymd_and_hms(1998, 5, 5, 12, 12, 0).unwrap(),
        ),
        // Case 12: Try for Reflector again
        (
            "Reflector Test 2",
            Utc.with_ymd_and_hms(1987, 10, 23, 1, 30, 0).unwrap(),
        ),
    ];
        ...
        match generate_hd_chart(*birth_time, "") {
            Ok(chart) => {
                ...
                println!(
                    ...
                    p_sun.gate, p_sun.line
                );
                println!(
                    "        \"personality_earth\": {{\"gate\": {}, \"line\": {}}},",
                    p_earth.gate, p_earth.line
                );
                println!(
                    "        \"design_sun\": {{\"gate\": {}, \"line\": {}}},",
                    d_sun.gate, d_sun.line
                );
                println!(
                    "        \"design_earth\": {{\"gate\": {}, \"line\": {}}},",
                    d_earth.gate, d_earth.line
                );
                ...
                // Active channels
                let mut channels: Vec<String> = chart
                    .channels
                    .iter()
