...
fn main() {
    ...
    match generate_hd_chart(birth_time, "") {
        Ok(chart) => {
            ...
            println!(
                "Authority: {:?} {} (expected: Sacral)",
                chart.authority,
                if auth_ok { "✓" } else { "✗" }
            );

            let profile_ok =
                chart.profile.conscious_line == 2 && chart.profile.unconscious_line == 4;
            println!(
                "Profile: {}/{} {} (expected: 2/4)",
            ...
            if let (Some(ps), Some(pe), Some(ds), Some(de)) = (p_sun, p_earth, d_sun, d_earth) {
                // Expected: 4/49 | 23/43
                let cross_matches = ((ps.gate == 4 && pe.gate == 49)
                    || (ps.gate == 49 && pe.gate == 4))
                    && ((ds.gate == 23 && de.gate == 43) || (ds.gate == 43 && de.gate == 23));
                println!(
                    "Cross gates: {}/{} | {}/{} {} (expected: 4/49 | 23/43)",
                    ps.gate,
                    pe.gate,
                    ds.gate,
                    de.gate,
