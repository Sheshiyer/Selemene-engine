...
use noesis_integration::{
    tcm_layer::TCMElement, verification::DataVerifier, BirthProfile, TCMAnalysis, UnifiedAnalysis,
};
...
fn test_shesh_date_parsing() {
    ...
    println!(
        "✓ Date parsing successful: {}-{:02}-{:02} {:02}:{:02}",
        date.year(),
        date.month(),
        date.day(),
        time.hour(),
...
async fn test_shesh_verification() {
    ...
    println!(
        "  Date format: {}",
        if date_check.matches { "✓" } else { "✗" }
    );
    println!(
        "  Time format: {}",
        if time_check.matches { "✓" } else { "✗" }
    );
    println!(
        "  Coordinates: {}",
        if coord_check.matches { "✓" } else { "✗" }
    );
}
...
fn test_shesh_tcm_analysis() {
    ...
    let analysis =
        TCMAnalysis::from_birth_profile(&profile).expect("Failed to generate TCM analysis");
    ...
    assert!(
        ...
        "Should have TCM recommendations"
    );

    // Should have optimal times
    assert!(
        !analysis.optimal_times.is_empty(),
        "Should have optimal time windows"
    );
}
    ...
    match UnifiedAnalysis::generate(&profile).await {
        Ok(analysis) => {
            ...
            println!(
                ...
                analysis.vimshottari.current_mahadasha
            );
            println!(
                "  Life Path Number: {}",
                analysis.numerology.life_path_number
            );
