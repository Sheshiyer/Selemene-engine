...
fn engine_matrix_matches_workspace_versions() {
    ...
    assert_eq!(matrix.rust_engines.len(), 12);
    assert_eq!(matrix.ts_engines.len(), 6);
    assert_eq!(matrix.counts.rust_engines, 12);
    assert_eq!(matrix.counts.ts_engines, 6);
    assert_eq!(matrix.counts.total_engines, 18);
...
fn dependency_graph_matches_cargo_metadata() {
    ...
    assert_eq!(graph.workspace_crate_count, 27);
