...
use engine_human_design::{Activation, HDChart, Planet};
...
mod tests {
    ...
    fn test_map_hd_to_gene_keys() {
        ...
        assert_eq!(
            pers_sun.key_number, 17,
            "Personality Sun should map to Gene Key 17"
        );
        ...
        assert_eq!(
            pers_earth.key_number, 18,
            "Personality Earth should map to Gene Key 18"
        );
        ...
        assert_eq!(
            design_sun.key_number, 45,
            "Design Sun should map to Gene Key 45"
        );
        ...
        assert_eq!(
            design_earth.key_number, 26,
            "Design Earth should map to Gene Key 26"
        );
    ...
    fn test_calculate_activation_sequences() {
        let hd_chart = create_test_hd_chart(17, 18, 45, 26);
        let sequences =
            calculate_activation_sequences(&hd_chart).expect("Should calculate sequences");

        // Life's Work: Personality Sun + Earth
        assert_eq!(
            sequences.lifes_work,
            (17, 18),
    ...
    fn test_extract_sun_earth_gates() {
        ...
        let result = extract_sun_earth_gates(&hd_chart).expect("Should extract gates");
    ...
    fn test_different_sequences() {
        ...
        for (ps, pe, ds, de) in test_cases {
            ...
            let seq = calculate_activation_sequences(&chart).expect("Should calculate sequences");
