...
fn get_gateway_config() -> (String, String) {
    ...
    let gateway_token = std::env::var("GATEWAY_TOKEN")
        .unwrap_or_else(|_| "your-gateway-token".to_string());
...
pub async fn create_invite(
    ...
    let code = format!("selemene_{}", Uuid::new_v4().to_string().replace("-", "")[..16].to_string());
...
pub async fn get_onboarding(
    ...
    let invite = codes.get(&code).ok_or_else(|| {
        (axum::http::StatusCode::NOT_FOUND, "Invite code not found")
    })?;
...
fn format_onboarding_text(invite_code: &str, gateway_url: &str, token: &str, _user_id: &str) -> String {
