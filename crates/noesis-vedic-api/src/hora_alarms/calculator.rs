...
pub fn calculate_day_horas(
    ...
    let start_idx = sequence.iter().position(|p| *p == day_ruler).unwrap_or(0);
...
pub fn get_current_hora(
    ...
    horas
        .iter()
        .find(|h| is_time_between(current_time, h.start_time, h.end_time))
}
...
pub fn get_next_hora_of_planet<'a>(
    ...
    horas
        .iter()
        .find(|h| h.ruler.eq_ignore_ascii_case(planet) && h.start_time >= after_time)
}
...
fn add_minutes(time: NaiveTime, minutes: u32) -> NaiveTime {
    ...
    NaiveTime::from_hms_opt((total_mins / 60) % 24, total_mins % 60, 0).unwrap_or(time)
}
