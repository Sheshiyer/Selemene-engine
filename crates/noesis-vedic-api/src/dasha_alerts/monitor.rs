...
use super::{DashaAlertConfig, DashaTransitionEvent, TransitionSignificance};
...
impl DashaTransitionMonitor {
    ...
    pub fn check_upcoming(
        &self,
        _check_date: NaiveDate,
        _days_ahead: u32,
    ) -> Vec<DashaTransitionEvent> {
    ...
    fn get_transition_advice(&self, _from: &str, to: &str) -> String {
        match to.to_lowercase().as_str() {
            "saturn" => {
                "Prepare for a period of hard work and discipline. Focus on long-term goals."
                    .to_string()
            }
            "jupiter" => {
                "A period of growth and opportunities begins. Stay ethical and optimistic."
                    .to_string()
            }
            "rahu" => {
                "Material desires may intensify. Stay grounded and avoid shortcuts.".to_string()
            }
            "ketu" => "Spiritual insights increase. Let go of attachments gracefully.".to_string(),
            "venus" => "Relationships and creativity flourish. Enjoy life's pleasures mindfully."
                .to_string(),
            "mars" => {
                "Energy and initiative increase. Channel aggression constructively.".to_string()
            }
            "mercury" => {
                "Communication and learning are favored. Start new courses or skills.".to_string()
            }
            "moon" => {
                "Emotional sensitivity heightens. Nurture yourself and loved ones.".to_string()
            }
            "sun" => {
                "Leadership opportunities arise. Step into authority with confidence.".to_string()
            }
...
mod tests {
    ...
    fn test_lord_significance() {
        ...
        assert_eq!(
            monitor.get_lord_significance("Saturn"),
            TransitionSignificance::Major
        );
        assert_eq!(
            monitor.get_lord_significance("Mercury"),
            TransitionSignificance::Minor
        );
    }
}
