...
const MARRIAGE_BAD_TITHIS: &[&str] = &["Amavasya", "Purnima", "Chaturthi", "Navami", "Chaturdashi"];
...
#[allow(dead_code)]
const MARRIAGE_BAD_MONTHS: &[&str] = &["Adhik Mas", "Pitru Paksha"];
...
pub fn marriage_recommendation(quality: MuhurtaQuality, _score: u8) -> String {
    match quality {
        MuhurtaQuality::Excellent => "Highly auspicious time for marriage ceremonies. \
             All major factors are favorable."
            .to_string(),
        MuhurtaQuality::Good => "Good muhurta for marriage. Minor unfavorable factors can be \
             mitigated with appropriate remedies."
            .to_string(),
        MuhurtaQuality::Average => "Average muhurta. Consider finding a better time if possible, \
             or perform remedial measures."
            .to_string(),
        MuhurtaQuality::NotRecommended => "This muhurta has significant unfavorable factors. \
             It's advisable to look for an alternative time."
            .to_string(),
        MuhurtaQuality::Avoid => "This time should be avoided for marriage. \
             Several major doshas are present."
            .to_string(),
    }
}
...
mod tests {
    ...
    fn test_good_marriage_muhurta() {
        let (quality, score, favorable, _) =
            evaluate_marriage_muhurta("Panchami", "Rohini", "Siddha", "Thursday", false, false);
    ...
    fn test_bad_marriage_muhurta() {
        let (quality, score, _, unfavorable) =
            evaluate_marriage_muhurta("Amavasya", "Bharani", "Vyatipata", "Saturday", true, true);
