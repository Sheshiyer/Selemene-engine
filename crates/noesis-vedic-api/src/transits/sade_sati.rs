...
use super::types::{SadeSatiPhase, SadeSatiStatus};
use crate::birth_chart::types::ZodiacSign;
use chrono::NaiveDate;

/// Check if Sade Sati is active
pub fn check_sade_sati(moon_sign: ZodiacSign, saturn_sign: ZodiacSign) -> SadeSatiStatus {
...
pub fn calculate_sade_sati_dates(
    ...
    let _sign_duration_days = 912; // ~2.5 years
    let _moon_num = moon_sign.number();
...
pub fn check_dhaiya(moon_sign: ZodiacSign, saturn_sign: ZodiacSign) -> Option<String> {
    ...
    match position {
        4 => Some(
            "Dhaiya (Ashtama Shani) - Saturn 4th from Moon. \
                  May cause health and domestic challenges."
                .to_string(),
        ),
        8 => Some(
            "Dhaiya (Kantaka Shani) - Saturn 8th from Moon. \
                  May bring obstacles and transformation."
                .to_string(),
        ),
        _ => None,
    }
